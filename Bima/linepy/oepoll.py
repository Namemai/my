...
class OEPoll(object):
    ...
    def __execute(self, bim, threading):
        try:
            if threading:
                _td = threading.Thread(target=self.OpInterrupt[bim.type](bim))
                _td.daemon = False
                _td.start()
            else:
                self.OpInterrupt[bim.type](bim)
    ...
    def trace(self, threading=False):
        ...
        for bim in operations:
            if bim.type in self.OpInterrupt.keys():
                self.__execute(bim, threading)
            self.setRevision(bim.revision)
